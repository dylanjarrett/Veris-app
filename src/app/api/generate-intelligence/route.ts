...
const client = new OpenAI({
  apiKey: process.env.VERIS_OPENAI_KEY,
});
...
export async function POST(req: Request) {
  try {
    ...
    if (!client.apiKey) {
      console.error("Missing VERIS_OPENAI_KEY");
      return NextResponse.json(
        { error: "Missing VERIS_OPENAI_KEY on server" },
        { status: 500 }
      );
    }
    ...
    const prompt = `
You are Veris, an AI assistant that writes real estate marketing copy for agents.
